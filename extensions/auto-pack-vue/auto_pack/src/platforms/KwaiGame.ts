...
export class KwaiGame extends BasePlatform {
    public async afterBuildFinish() {
        ...
        if (existsSync(configPath)) {
            ...
            let adapterpath = path.join(this.outputPath, channelInfo.platform);
            let gamejspath = path.join(this.outputPath,channelInfo.platform,"game.js");
            let t = readFileSync(gamejspath, { encoding: 'utf-8' });
            t = `require('./kwaiadapter.js');`+t;
            writeFileSync(gamejspath, t,{encoding:'utf-8'});
            console.log('start copy kwaiadapter.js', adapterpath);
            PackUtil.copyFiles(path.join(__dirname, "../", "kwaiadapter.js"), adapterpath);
            console.log('end copy adpater-min.js');
