...
import { generateInterface } from "./InterfaceGenerator";
...
export default class CfgUtils {
    ...
    public static parseExcel(excelPaths: string[], exportPath: string, exportTSPath: string, isFormat: boolean, isArr: boolean, cb: Function) {
        if (excelPaths) {
    ...
    public static writeFileJson(exportPath: string, sheet: { data: unknown, name: string }, isFormat: boolean, isArr: boolean) {
    ...
    static writeFileTS(exportPath: string, sheet: { data: unknown, name: string }) {
        if (exportPath === '') return;
        let { data, name } = sheet;
        // 提取表头(第0行)和规则(第2行)
        const titles = data[0];
        const rules = data[2];
        if (titles && rules) {
            const tsContent = generateInterface(name, titles, rules);
            const tsFilePath = path.join(exportPath, `${name}.ts`);
            writeFileSync(tsFilePath, tsContent);
        }
    }

    public static getJsonData(excelData: any, isArr: boolean) {
