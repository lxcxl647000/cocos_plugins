...
class BuildTaskAndroid {
    ...
    copyLibraryDirectory(opts) {
        const dest = AndroidConstants_1.AndroidConstants.ProjectLibAdMobDir;
        const overwrite = opts.overwriteLibrary !== false;
        if (fse.existsSync(dest) && !overwrite) {
            return;
        }
        fse.copySync(AndroidConstants_1.AndroidConstants.TemplateLibAdMobDir, dest, { overwrite: true });
    }
    /**
     ...
     */
    patchLibraryBuildGradle(opts) {
        var _a, _b;
        const buildGradlePath = `${AndroidConstants_1.AndroidConstants.ProjectLibAdMobDir}/build.gradle`;
        ...
        if (!content.includes("namespace ")) {
            ...
            content = content.replace(/apply plugin: ['"]com\.android\.library['"]/i, 'apply plugin: \'com.android.library\'\n\n    namespace \'com.cocosext.admob\'');
        }
        ...
        content = content.replace(/manifestPlaceholders\s*=\s*\[[\s\S]*?\]\s*(?=\s*\}\s*$|\s*defaultConfig)/g, '');
        ...
        if (defaultConfigMatch) {
            ...
            if (!defaultConfigBlock.includes('manifestPlaceholders')) {
                ...
                const testDeviceIds = ((_a = opts.testDeviceIds) !== null && _a !== void 0 ? _a : '').replace(/"/g, '\\\"');
        ...
        manifestPlaceholders = [
            ...
            admobTestDeviceIds: "${testDeviceIds}",
                ...
                content = content.replace(/(defaultConfig\s*\{[\s\S]*?)\n(\s*\})/, `$1${placeholderBlock}\n$2`);
                // Also remove any manifestPlaceholders that might be at android {} top level
                // (between android { and defaultConfig {)
                content = content.replace(/(android\s*\{[\s\S]*?)manifestPlaceholders\s*=\s*\[[\s\S]*?\]\s*(?=\s*defaultConfig)/, '$1');
            }
        }
        else {
            ...
            const appId = (opts.androidAppId && opts.androidAppId.trim()) || TEST_ANDROID_APP_ID;
            const useTestAds = opts.useTestAds !== false;
            const testDeviceIds = ((_b = opts.testDeviceIds) !== null && _b !== void 0 ? _b : '').replace(/"/g, '\\\"');
            const body = `        manifestPlaceholders = [\n` +
                `            admobAppId: "${appId}",\n` +
                `            admobUseTestAds: "${useTestAds}",\n` +
                `            admobTestDeviceIds: "${testDeviceIds}",\n` +
                `        ]`;
            (0, Util_1.upsertMarkerBlock)(buildGradlePath, MANIFEST_PLACEHOLDER_BEGIN, MANIFEST_PLACEHOLDER_END, body, ANDROID_BLOCK_ANCHOR);
        }
    ...
    patchSettingsGradle() {
        ...
        const possiblePaths = [
            AndroidConstants_1.AndroidConstants.ProjectBuildSettingsGradle, // build/android/proj/settings.gradle
            AndroidConstants_1.AndroidConstants.ProjectSettingsGradle, // native/engine/android/settings.gradle
        ];
        ...
        if (!content.includes("'libadmob'") && !content.includes('"libadmob"')) {
            ...
            if (includeLines) {
                ...
                if (content.includes('rootProject.name')) {
                    content = content.replace(/(rootProject\.name\s*=)/, `include ':libadmob'\nproject(':libadmob').projectDir = new File(NATIVE_DIR, 'libadmob')\n\n$1`);
                }
                else {
                    content = `${content}\ninclude ':libadmob'\nproject(':libadmob').projectDir = new File(NATIVE_DIR, 'libadmob')\n`;
                }
            ...
            if (!content.includes("project(':libadmob').projectDir")) {
                if (content.includes('rootProject.name')) {
                    content = content.replace(/(rootProject\.name\s*=)/, "project(':libadmob').projectDir = new File(NATIVE_DIR, 'libadmob')\n\n$1");
                }
                else {
                    content = `${content}\nproject(':libadmob').projectDir = new File(NATIVE_DIR, 'libadmob')\n`;
                }
    ...
    patchAppBuildGradle() {
        const snippet = fse.readFileSync(AndroidConstants_1.AndroidConstants.TemplateProjectBuildGradle, 'utf-8').trimEnd();
        (0, Util_1.insertContentBehindKey)(AndroidConstants_1.AndroidConstants.ProjectAppBuildGradle, DEPENDENCIES_ANCHOR, snippet);
    }
    /**
     ...
     */
    patchMinSdkVersion() {
        const propsPath = AndroidConstants_1.AndroidConstants.ProjectBuildGradleProperties;
        if (!fse.existsSync(propsPath)) {
            ...
            const fallbackPath = `${AndroidConstants_1.AndroidConstants.ProjectNativeAndroidDir}/gradle.properties`;
     ...
     */
    patchMinSdkInBuildGradle() {
        const appBuildGradle = AndroidConstants_1.AndroidConstants.ProjectAppBuildGradle;
     ...
     */
    patchProguardRules() {
        const rulesPath = AndroidConstants_1.AndroidConstants.ProjectAppProguardRules;
        if (!fse.existsSync(rulesPath)) {
            // Try build output path
            const buildRulesPath = AndroidConstants_1.AndroidConstants.ProjectBuildAppProguardRules;
...
exports.buildTaskAndroid = { android: new BuildTaskAndroid() };
